...
1) The `data/m13/` directory contains flats, darks, raw observations, and reduced/combined data for the B, V, and R data bands for observations of the Hercules Cluster. The subsequent `notebooks/hecules_photometry.ipynb` notebook shows a walkthrough of standard data processing, after which the stacked images in each filter can be used to create a color image in the software of the user's choosing (i.e. `ds9` or `astropy`). RGB Image of the Hercules Cluster created using the data in this repository:
